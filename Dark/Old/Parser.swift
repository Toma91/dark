...
struct Parser<Buffer: RandomAccessCollection> where Buffer.Element == Character {
...
extension Parser {
    ...
    mutating func lex() {
        let leadingTrivia = lexLeadingTrivia()
        
        guard index < buffer.endIndex else { fatalError("Unexpected end of file") }
        
        let flavour: Token.Flavour
        
        switch buffer[index] {

        case "a" ... "z": fallthrough
        case "A" ... "Z": fallthrough
        case "_": flavour = lexIdentifier()
            
        default: fatalError("Unknown character '\(buffer[index])'")
            
        }
        
        print(leadingTrivia, flavour)
...
private extension Parser {
    ...
    mutating func lexIdentifier() -> Token.Flavour {
        var text: String = "\(buffer[index])"
       
        buffer.formIndex(after: &index)

        lexingLoop: while index < buffer.endIndex {
            switch buffer[index] {
            
            case "a" ... "z": fallthrough
            case "A" ... "Z": fallthrough
            case "0" ... "9": fallthrough
            case "_": text.append(buffer[index])
                
            default: break lexingLoop
...
private extension Parser {
    ...
    mutating func lexLeadingTrivia() -> [Trivia] {
        var result: [Trivia] = []
